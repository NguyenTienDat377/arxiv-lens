...
**The first design put that state in an `actions/cache`, and it deadlocked.** Run one finds an empty cache, sees 300 new papers, is refused by the guard, and therefore never reaches the step that *saves* the cache. Run two is identical. The only escape was to pay full price once to seed it — which would have made a best-effort cache, evicted after seven days unused, load-bearing for correctness. A cache is an optimisation; the moment something cannot bootstrap without one, it is not a cache any more.

So `data/extracted/` is committed: 448K of JSONL, 300 papers, two snapshots, and the one exception to the `data/` ignore rule. A fresh checkout has the reuse index *and* a drift baseline before any step runs. The cache stays, carrying new extractions forward, but now purely as an optimisation on a floor that cannot disappear. (Git will not re-include a path under an excluded directory, so the rule has to un-ignore `graph-pipeline/data/`, re-exclude `graph-pipeline/data/*`, then un-exclude `extracted/`.)

**The guard covers what the floor cannot.** Caches get evicted and arXiv keeps publishing, so `--max-new` makes an unexpectedly large run loud: above N pending papers `extract_entities` exits non-zero before `submit_batch` is ever called. Verified by pointing it at a 500-paper snapshot with 300 already extracted — `refusing: 200 new papers exceeds --max-new 5`, exit 1, no batch created. Roughly two dollars not spent.
